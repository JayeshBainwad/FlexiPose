...
class ShoulderExercise(context: Context?, attrs: AttributeSet?) : View(context, attrs) {
    ...
    override fun draw(canvas: Canvas) {
        ...
        results?.let { poseLandmarkerResult ->
            ...
            for (landmark in poseLandmarkerResult.landmarks()) {
                ...
                calculateLeftShoulderAngleAndReps(canvas)
            }
        }
    }
...
//    }
    ...
    fun calculateLeftShoulderAngleAndReps(canvas: Canvas) {
        ...
        results?.let { poseLandmarkerResult ->
            ...
            // Update UI with the angle
            post {
                bindingCameraActivity?.tvAngle?.text = "Left Shoulder Angle: $angleCount"
            }
    ...
    private fun trackRepLeftShoulder(angle: Double) {
        ...
        // Detect flexing and extension
        if (angle > 160 && !isFlexing) {
            isFlexing = true // Shoulder flexing phase started
            repMaxAngles.add(currentRepMaxAngle)
        } else if (angle < 90 && isFlexing) {
    ...
    private fun storeExerciseData() {
        ...
        FirestoreClass().storeExerciseData(context as CameraActivity, exerciseInfo, "ShoulderExercise")
    }
